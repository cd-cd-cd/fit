...
# 对称交叉熵损失
def contrastive_loss2(v1, v2, clip_model):
    ...
    ground_truth = torch.arange(len(v1)).long().to(device)
    # Image loss.
    logits_per_image =  logit_scale * v1 @ v2.t()
    loss_img_val = loss_img(logits_per_image, ground_truth)
    logits_per_text = logit_scale * v2 @ v1.t()
...
def prompt_token_replace(local_attr_num, text, img_subj_token, img_attr_tokens, clip_model, map_type=0):
    """
    Replace symbols * with subject-oriented pseudo-word token or attribute-oriented pseudo-word token:
    map_type=0,replace both
    map_type=1,replace * with subject-oriented pseudo-word token
...
def encode_text_img_learnable(x, collect_ind, clip_model):
    x = x + clip_model.positional_embedding.type(clip_model.dtype)
    x = x.permute(1, 0, 2)
    x = clip_model.transformer(x)
    x = x.permute(1, 0, 2)
...
def freeze_parameters(model, params_to_freeze):
    for name, param in model.named_parameters():
        if any(substring in name for substring in params_to_freeze):
