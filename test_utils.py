...
def main():
    ...
    parser.add_argument("--dataset", type=str, choices=['cirr', 'fashioniq', 'circo'], default="fashioniq",
                        help="Dataset to use")
    # test dataset path
    parser.add_argument('--Fashion_IQ_path', type=str, default="/amax/home/chendian/dataset/fashionIQ")
    parser.add_argument('--CIRR_path', type=str, default="/amax/home/chendian/dataset/cirr_dataset")
...
def test(
         ...
         save_path="/amax/home/chendian/FTI4CIR-main/save_model",
    ...
    if dataset.lower() == 'fashioniq':
        # Extract the index features
        classic_val_dress_dataset = FashionIQDataset(dataset_path, 'val', ["dress"], 'classic', preprocess)
        index_dress_features, index_dress_names = extract_index_features(classic_val_dress_dataset, clip_model)
        # Define the relative validation dataset
        relative_val_dress_dataset = FashionIQDataset(dataset_path, 'val', ["dress"], 'relative', preprocess)

        # Extract the index features
        classic_val_shirt_dataset = FashionIQDataset(dataset_path, 'val', ["shirt"], 'classic', preprocess)
        index_shirt_features, index_shirt_names = extract_index_features(classic_val_shirt_dataset, clip_model)
        # Define the relative validation dataset
        relative_val_shirt_dataset = FashionIQDataset(dataset_path, 'val', ["shirt"], 'relative', preprocess)

        # Extract the index features
        classic_val_toptee_dataset = FashionIQDataset(dataset_path, 'val', ["toptee"], 'classic', preprocess)
        index_toptee_features, index_toptee_names = extract_index_features(classic_val_toptee_dataset, clip_model)
        # Define the relative validation dataset
        relative_val_toptee_dataset = FashionIQDataset(dataset_path, 'val', ["toptee"], 'relative', preprocess)

    elif args.dataset.lower() == 'cirr':
        # Extract the index features
        classic_test_dataset = CIRRDataset(dataset_path, 'test', 'classic', preprocess)
        index_features, index_names = extract_index_features(classic_test_dataset, clip_model)
        relative_test_dataset = CIRRDataset(dataset_path, 'test', 'relative', preprocess)
    elif args.dataset.lower() == 'circo':
        # Extract the index features
    ...
    if dataset.lower() == 'fashioniq':
        ...
        dress_10, dress_50 = evaluate.compute_fiq_val_metrics(relative_val_dress_dataset, clip_model, img2text,
                                                              index_dress_features, index_dress_names)
        average_recall10_list.append(dress_10)
        average_recall50_list.append(dress_50)
        
        shirt_10, shirt_50 = evaluate.compute_fiq_val_metrics(relative_val_shirt_dataset, clip_model, img2text,
                                                              index_shirt_features, index_shirt_names)
        average_recall10_list.append(shirt_10)
        average_recall50_list.append(shirt_50)
        
        toptee_10, toptee_50 = evaluate.compute_fiq_val_metrics(relative_val_toptee_dataset, clip_model, img2text,
                                                                index_toptee_features, index_toptee_names)
        average_recall10_list.append(toptee_10)
        average_recall50_list.append(toptee_50)
        
        metrics = {"dress_10": dress_10, "dress_50": dress_50,
                   "shirt_10": shirt_10, "shirt_50": shirt_50,
                   "toptee_10": toptee_10, "toptee_50": toptee_50,
                   "average_10": mean(average_recall10_list), "average_50": mean(average_recall50_list),
                   "average_recall": (mean(average_recall10_list) + mean(average_recall50_list)) / 2
                   }
        
        return metrics

    elif dataset.lower() == 'cirr':
        # generate the test results
        generate_test_json.cirr_generate_test_submission_file(img2text, clip_model, index_features, index_names,
                                                              relative_test_dataset,
                                                              f"cirr", save_path)

    elif dataset.lower() == 'circo':
        # generate the test results
        generate_test_json.circo_generate_test_submission_file(img2text, clip_model, index_features,
                                                               index_names,
                                                               relative_test_dataset,
